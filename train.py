...
- Cropping traning data set from raw image
- Data augmentation
- Data incrementation
- Train classifiers
...
# 2. Cropping the face from the raw images    
img_target = []
for img in label:
    idx = int(img[0])
    o_h = int(img[1])
    ...
    # Data augmentation by flipping the image horizontally
...
# 3. Calculate the size of a sliding window based on the face images
mean_hl = np.mean(label[:,3]/label[:,4])
ratio_HL = 1.5

# 4. Define the size of the sliding window the resize the face accordingly
l_fixed = 40
h_fixed = 60
...
# 5. Negative examples: background
# 5.1. Calculate the IoU
...
# 5.2  Generate negative examples
# We define IoU > 0.5 as positive
data_train_neg = []
